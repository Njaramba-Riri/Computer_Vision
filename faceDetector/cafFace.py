...
proto = os.path.join(cd, "models/deploy.prototxt")
...
def real_timeDetection(model=model, source=s):
    ...
    while cap.isOpened():
        ...
        cv.putText(frame, label, (5, 30), cv.FONT_HERSHEY_SIMPLEX, .5, (0, 255, 0))
        frames.append(frame)
